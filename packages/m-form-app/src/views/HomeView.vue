<template>
  <div id="page">
    <FormProvider v-if="schemadata" :form="form">
...
import { getFormSchema, getData, Initialize } from "../api/getSchema";
import { SubmitValue, SchemaData } from "../utils/type";
...
// 判断是否是预览模式
const isTemplate: Ref<boolean> = ref(true);
...
// 获取对应 落地页 code值
const pageId: LocationQueryValue | LocationQueryValue[] =
  route.query.formId || route.query.templateId;

// 初始化 schema 数据
const schemadata: SchemaData = reactive({
  code: "",
  Message: "",
  data: { schema: {}, form: {} },
});
...
// 判断是否要触发提交
const bolen: Ref<boolean> = ref(true);
...
const useEffects = () => {
  ...
  onFormSubmit(async () => {
    if (bolen.value) {
      const resule: SubmitValue = await Initialize(pageId, form.values);
      if (resule.code === "200") {
        Notify({
          message: "提交成功",
          type: "success",
...
// 创建表单
const form: typeForm<any> = createForm({ effects: useEffects });
